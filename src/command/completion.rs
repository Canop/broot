...
impl Completions {
    ...
    fn for_verb(
        ...
        match con.verb_store.search(start, Some(sel_info), false, panel_state_type) {
            ...
            PrefixSearchResult::Matches(completions) => {
                Self::for_wholes(start, completions)
            }
        }
    }
    ...
    fn list_for_path(
        ...
        let anchor = match con.verb_store.search(
            verb_name,
            Some(sel_info),
            false,
            panel_state_type,
        ...
        let (_, parent_part, child_part) =
            regex_captures!(r"^(.*?)([^/]*)$", arg).unwrap();
