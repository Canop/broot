...
impl VerbStore {
    ...
    fn add_builtin_verbs(&mut self) -> Result<(), ConfError> {
        ...
        self.add_internal(close_panel_cancel).with_key(key!(ctrl - w));
        #[cfg(unix)]
        self.add_external(
            "copy {newpath}",
        ...
        self.add_internal(focus)
            ...
            .with_key(key!(ctrl - f));
        self.add_internal(help).with_key(key!(F1)).with_shortcut("?");
        #[cfg(feature = "clipboard")]
        ...
        .with_auto_exec(false)
        .with_key(key!(f2));
        self.add_internal_bang(start_end_panel).with_key(key!(ctrl - p));
        ...
        self.add_internal(open_stay).with_key(key!(enter)).with_shortcut("os");
        self.add_internal(open_stay_filter).with_shortcut("osf");
        self.add_internal(parent).with_key(key!(h)).with_shortcut("p");
        self.add_internal(page_down).with_key(key!(ctrl - d)).with_key(key!(pagedown));
        self.add_internal(page_up).with_key(key!(ctrl - u)).with_key(key!(pageup));
        self.add_internal(panel_left_no_open).with_key(key!(ctrl - left));
        ...
        self.add_internal(toggle_ignore).with_key(key!(alt - i)).with_shortcut("gi");
        ...
        self.add_internal(toggle_hidden).with_key(key!(alt - h)).with_shortcut("h");
        #[cfg(unix)]
            ...
            (Some(ep), None, None, None) => {
                if let Some(internal_pattern) = ep.as_internal_pattern() {
                        ...
                        self.verbs.iter().find(|&v| v.has_name(internal_pattern))
                    {
                        ...
                        VerbExecution::Internal(InternalExecution::try_from(
                            internal_pattern,
                        )?)
                    }
                } else {
