use {
    super::{detect_support::is_kitty_graphics_protocol_supported, terminal_esc::get_esc_seq},
    base64::{
...
impl<'i> KittyImage<'i> {
    ...
    fn print_with_chunks(
        ...
        let mut pos = 0;
        loop {
            if self.is_tmux { tmux_write_header!(w)?; }
            if pos + CHUNK_SIZE < encoded.len() {
                ...
                pos += CHUNK_SIZE;
                if self.is_tmux { tmux_write_tail!(w)?; }
            } else {
                // last chunk
                write!(w, "{}_Gq=2,m=0;{}{}\\", &esc, &encoded[pos..encoded.len()], &esc)?;
                if self.is_tmux { tmux_write_tail!(w)? };
                // display image
                if self.is_tmux { tmux_write_header!(w)? };
                write!(
