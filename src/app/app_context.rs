...
impl AppContext {
    pub fn from(
        ...
        let search_modes = config
            ...
            .unwrap_or_default();
        let ext_colors =
            ExtColorMap::try_from(&config.ext_colors).map_err(ConfError::from)?;
        ...
        let max_staged_count =
            config.max_staged_count.unwrap_or(10_000).clamp(10, 100_000);
        ...
        let reset_terminal_title_on_exit =
            config.reset_terminal_title_on_exit.unwrap_or(false);
        let preview_transformers =
            PreviewTransformers::new(&config.preview_transformers)?;
        ...
        Ok(Self {
            ...
            kitty_graphics_transmission: config
                .kitty_graphics_transmission
                .unwrap_or_default(),
            kept_kitty_temp_files,
            lines_after_match_in_preview: config
                .lines_after_match_in_preview
                .unwrap_or(0),
            lines_before_match_in_preview: config
    ...
    pub fn cmd(&self) -> Option<&str> {
        self.launch_args
            .cmd
            .as_ref()
            .or(self.config_default_args.as_ref().and_then(|args| args.cmd.as_ref()))
            .map(String::as_str)
    }
...
fn initial_root_file(
    cli_args: &Args
) -> Result<(PathBuf, Option<PathBuf>), ProgramError> {
    let mut file = None;
