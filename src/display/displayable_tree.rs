...
impl<'a, 's, 't> DisplayableTree<'a, 's, 't> {
    ...
    fn write_line_label<W: Write>(
        ...
        if pattern_object.subpath {
            ...
            {
                let name_match =
                    self.tree.options.pattern.pattern.find_string(&line.subpath);
            ...
            if line_index < tree.lines.len() {
                ...
                for col in &visible_cols {
                    let void_len = match col {
                        ...
                        Col::Date => {
                            if let Some(seconds) =
                                line.sum.and_then(|sum| sum.to_valid_seconds())
                            {
