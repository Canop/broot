...
impl TokPattern {
    pub fn new(pattern: &str) -> Self {
        ...
        let mut toks: Vec<Box<[char]>> = if let Some(sep) = sep {
            pattern.split(sep).filter(|s| !s.is_empty()).map(norm_chars).collect()
    ...
    pub fn score_of(
        ...
        self.find_ranges(candidate).map(|_| self.score_of_matching(candidate))
    }
}
...
mod tok_pattern_tests {
    ...
    fn check_pos(
        ...
        let target_pos: Pos =
            pos.chars().enumerate().filter(|(_, c)| *c == '^').map(|(i, _)| i).collect();
